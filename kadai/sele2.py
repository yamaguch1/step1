...
def main():
    ...
    while True:
        ...
        status_list = driver.find_elements_by_css_selector(".cassetteRecruit__heading .labelEnploymentStatus")
        table_list = driver.find_elements_by_css_selector(".cassetteRecruit__heading .tableCondition")#初年度年収
    ...
    #現在時刻を指定した文字列フォーマットとして取得（よく使う）
    now = datetime.datetime.now().strftime('%Y-%m-%d-%M^%S')
    # pandas形式のデータを作成（DateFrame）
